...
//  Processa dados do Arquivo CPU.MIF
    ...
    printf("Rodando...                             ");
    printf("Tecle <HOME> para Menu                    ");
    printf("Tecle <END> para executar linha atual    ");
...
voltah:     key = getchar();        // Wait for user input
   ...
   if(step == 1) {
        printf("PC: %05d  IR: %05d   SP: %05d   FR: %d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d", pc, mem[pc], sp, FR[15], FR[14], FR[13], FR[12], FR[11], FR[10], FR[9], FR[8], FR[7], FR[6], FR[5], FR[4], FR[3], FR[2], FR[1], FR[0]);
        printf("R0: %06d  R1: %06d   R2: %06d   R3: %06d", reg[0], reg[1], reg[2], reg[3] );
        printf("R4: %06d  R5: %06d   R6: %06d   R7: %06d", reg[4], reg[5], reg[6], reg[7] );


pausa:  key = getchar();        // Wait for user input
      	if (key == 'b') goto loop;
        if (key == 'a') goto volta2;
        goto pausa;
        }
